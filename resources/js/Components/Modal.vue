        ...
        <Dialog as="div" class="fixed z-[70] inset-0 overflow-y-auto" @close="close">
          <div v-bind="$attrs" class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <TransitionChild as="template" enter="ease-out duration-300" enter-from="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" enter-to="opacity-100 translate-y-0 sm:scale-100" leave="ease-in duration-200" leave-from="opacity-100 translate-y-0 sm:scale-100" leave-to="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
              <div :class="getSize()" class="relative inline-block bg-white dark:bg-dark-box rounded-lg  text-left overflow-hidden shadow-xl transform transition-all sm:my-8 w-full align-middle sm:align-middle sm:w-full">
                <header v-if="!asAlert" class="bg-gradient-to-r from-green-600 via-churpy-green to-dark-box/90 dark:bg-gray-700 m-0 px-3 pt-3 pb-2 flex justify-between text-white px-4">
                  <h3>
